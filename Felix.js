...
var SmartVelocity = function () {
  this.getName = function () {
    return "SmartVelocity" + suffix;
  };
  this.onEnable = function () {
    return localPlayer.sendMessage(".hide velocity");
  };
  this.onPreUpdate = function () {
  ...
  this.onDisable = function () {
    return localPlayer.sendMessage(".show velocity");
  };
};
/**
 * =============================
...
var FelixCommands = function () {
  this.getName = function () {
    return "FelixCommands";
  };
  this.onSendPacket = function (id, data) {
    return Handler.listen(id, data);
  };
  this.onPreUpdate = function () {
...
var TargetHud = function () {
  ...
  this.onRender2D = function (w, h, partialTicks) {
    ...
    if (!moduleManager.getBoolean("RePosition")) {
      renderHud(colorArray, target, g, X);
      return;
    }
...
function renderHud(colorArray, target, g, X) {
  var healthRGB = healthColour(target.health);
  ...
  if (moduleManager.getBoolean("Health-colour"))
    render.drawRect(anchorX + 8, anchorY + 36, anchorX + progressLength(target.health), anchorY + 42, healthRGB[0], healthRGB[1], healthRGB[2], 255);
  else
    drawGradientRect(anchorX + 8, anchorY + 36, 145, 6, g, {
      alpha: 255,
      angle: 0,
      offset: X,
...
//I could have made a gradient for this, but lazy
...
function progressLength(health) {
  var fullLength = 152,
    increment = fullLength / 20;
  return increment * +health;
}
//took this from michaelxf
function drawGradientRect(x, y, width, height, color, options) {
  var rect = {
    x: x,
    y: y,
    width: width,
  ...
  if (rect) {
    ...
    for (var x = 0; x < width; x += boxSize) {
      if (x > progressLength(options.health) - 7)
        continue;
      for (var y = 0; y < height; y += boxSize) {
        var px = x / width;
        var py = y / height;
        var pt = px * cos + py * sin + options.offset;
        var boxColor = gradient(color, pt % 1);
        render.drawRect(x1 + x, y1 + y, x1 + x + boxSize, y1 + y + boxSize, boxColor[0], boxColor[1], boxColor[2], alpha);
      }
    }
  }
};
...
function EntityFromFOV() {
  var cPos = localPlayer.getPosition(),
    pitch = localPlayer.getAngles()[0],
    yaw = localPlayer.getAngles()[1],
    gran = 5,
    x = cPos[0],
    y = cPos[1] + 1.62,
    z = cPos[2],
    xShift = -1 * (Math.cos(pitch * Math.PI / 180)) * Math.sin(yaw * Math.PI / 180),
    yShift = -1 * Math.sin(pitch * Math.PI / 180),
    zShift = (Math.cos(pitch * Math.PI / 180)) * Math.cos(yaw * Math.PI / 180),
...
var Jitter = function () {
  ...
  this.onRender2D = function () {
    ...
    if (moduleManager.getBoolean("NoParkinsons") && BlockFromFov())
      return;
    var angles = localPlayer.getAngles(),
      yaw = angles[0] + ((Math.random() * jitterX - Math.random() * jitterX) / 2),
      pitch = angles[1] + ((Math.random() * jitterY - Math.random() * jitterY) / 2);
 ...
 * =============================
 */
var modA = new GodBridge(),
  modB = new LegitIndicator(),
  modC = new SmartVelocity(),
  modD = new FelixCommands(),
  modE = new TargetHud(),
  modF = new Jitter(),
  modG = new SmartReach();
...
moduleManager.registerBoolean("FOVHud" + suffix, "Health-colour", "Health bar changes color based on their health", false);
